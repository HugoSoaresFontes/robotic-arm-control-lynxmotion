...
def detect_markers(gray_img, grid_size, min_marker_perimeter=40, aperture=11, visualize=False):
    # Usa o Threshold adaptativo, isto é, define um limite para qual os pixeis serão 0 e a partir do qual serão 1
    # Nesse caso o limite é definido por a média local de uma janela de 11 pixeis subtraída pela constante 9
    edges = cv2.adaptiveThreshold(gray_img, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, aperture, 9)

    # Usa o filtro canny para detectar bordas
    ...
    # cv2.drawContours(gray_img, contours, -1, (255, 0, 0))
    # cv2.drawContours(gray_img, contained_contours, -1, (0, 255, 0))
    ...
    for r in rect_cand:
        if correct_gradient(gray_img, r):
            ...
            if marker is not None:
                ...
                marker = {'id': msg, 'id_confidence': id_confidence, 'verts': r.tolist(), 'soft_id': soft_msg,
