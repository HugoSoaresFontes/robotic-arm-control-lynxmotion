...
def project_distort_pts(pts_xyz, camera_matrix, dist_coefs, rvec=np.array([0, 0, 0], dtype=np.float32),
...
def dist_pts_ellipse(ellipse, points):
    ...
    scaled_error = np.transpose(
        pts.T * ratio)  # per vector scalar multiplication: makeing sure that boradcasting is done right
...
def get_system_info():
    ...
    return "User: {}, Platform: {}, Machine: {}, Release: {}, Version: {}".format(username, sysname, nodename, release,
                                                                                  version)
